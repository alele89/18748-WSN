...
void zb_sched_loop_iteration() /* ZB_KEIL_REENTRANT */
{
  ...
  do
  {
...
#endif
        ZB_RING_BUFFER_FLUSH_GET(&sched.cb_q);
      }
    }
    ...
    if (sched.tm_queue!=0)
    {
      ...
      while ((ent = ZB_LIST_GET_HEAD(sched.tm_queue, next)) != NULL
...
*/
    ...
    if (sched.tm_queue)
    {
      ZB_TIMER_START(ZB_TIME_SUBTRACT(ZB_LIST_GET_HEAD(sched.tm_queue, next)->run_time,
                                      t));
    }
    else
    {
...
{
  zb_ret_t ret = RET_OK;
  zb_mac_cb_ent_t *ent = ZB_RING_BUFFER_PUT_RESERVE(&sched.mac_tx_q);
  if (ent)
  {
    ent->func = func;
    ent->param = param;
...
{
  zb_ret_t ret = RET_OK;
  zb_mac_cb_ent_t *ent = ZB_RING_BUFFER_PUT_RESERVE(&sched.mac_cb_q);
  if (ent)
  {
    ent->func = func;
    ent->param = param;
    ZB_RING_BUFFER_FLUSH_PUT(&sched.mac_cb_q);
    TRACE_MSG(TRACE_COMMON2, "%p scheduled mac cb %p param %hd (in_b %hd)",
              (FMT__P_P_H_H, ent, ent->func, ent->param, (!param ? (zb_uint8_t)-1 : ZB_BUF_FROM_REF(param)->u.hdr.is_in_buf)));
    ZB_ASSERT(param <= ZB_IOBUF_POOL_SIZE);
  }
...
{
  zb_ret_t ret = RET_OK;
  zb_cb_q_ent_t *ent = ZB_RING_BUFFER_PUT_RESERVE(&sched.cb_q);
...
{
  ZB_LIST_INSERT_HEAD(sched.tm_queue, next, nent);
}
...
zb_ret_t zb_schedule_alarm(zb_callback_t func, zb_uint8_t param, zb_time_t run_after) /* __reentrant for sdcc, to save DSEG space */
{
  ...
  if (nent !=0)
  {
    ...
    if ((sched.tm_queue==0)             /* queue is empty */
        || ZB_TIME_GE(
          ZB_LIST_GET_HEAD(sched.tm_queue, next)->run_time, nent->run_time)) /* new time is before
                                                                                  * queue start */
    {
    ...
    else
    {
      ...
      if ( ent != 0 )
      {
        ...
        if ( ent != 0)
        {
          ...
          ZB_LIST_INSERT_AFTER(sched.tm_queue, next, ent, nent);
        }
        else
        {
          /* insert head */
      ...
      {
        /* insert tail */
        ZB_LIST_INSERT_TAIL(sched.tm_queue, next, nent);
      }
    }

#if 0
    TRACE_MSG(TRACE_COMMON3, "current time %d", (FMT__D, t));
    ZB_LIST_ITERATE(sched.tm_queue, next, ent)
    {
...
zb_ret_t zb_schedule_alarm_cancel(zb_callback_t func, zb_uint8_t param) /* __reentrant for sdcc, to save DSEG space */
{
  ...
  for (ent = ZB_LIST_GET_HEAD(sched.tm_queue, next) ; ent ; ent = next)
  {
